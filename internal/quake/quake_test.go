...
func TestQuakeProto(t *testing.T) {
	...
	if f, err = os.Open("etc/2015p768477.pb"); err != nil {
		return
	}
	defer f.Close()
...
func BenchmarkUnmarshalQuakeXML(b *testing.B) {
	var f *os.File
	var by []byte
	var err error
	...
	if err = xml.Unmarshal(by, e); err != nil {
		b.Fatal(err)
	}

	if e.PublicID != "2015p768477" {
		b.Fatalf("expected publicID 2015p768477 got %s", e.PublicID)
	}
...
func BenchmarkUnmarshalQuakeJSON(b *testing.B) {
	var f *os.File
	var by []byte
	var err error
	...
	if f, err = os.Open("etc/2015p768477.json"); err != nil {
		b.Fatal(err)
	}
	defer f.Close()
	...
	if err = json.Unmarshal(by, e); err != nil {
		b.Fatal(err)
	}
...
func BenchmarkUnmarshalQuakeProtobuf(b *testing.B) {
	var f *os.File
	var err error
	var by []byte
	...
	if err = proto.Unmarshal(by, e); err != nil {
		b.Fatal(err)
	}
	...
	for n := 0; n < b.N; n++ {
		// ignore errors
		proto.Unmarshal(by, e)
	}
}

/*
Reads a SeisCompML 07 file, makes a quake protobuf file from it and writes it to disk.
Reads it in and returns the bytes in it.
*/
func create() (err error) {
	...
	if f, err = os.Open("../seiscompml07/etc/2015p768477.xml"); err != nil {
		return
	}
	defer f.Close()
